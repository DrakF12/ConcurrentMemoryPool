...
//设置一个公共的FreeList对象链表，每个对象中含有各个接口，到时候直接使用接口进行操作，这是一个栈
//让一个类来管理自由链表
class Freelist
{
private:
...
public:
	// 控制在12%左右的内碎片浪费
	// [1,128]				8byte对齐 freelist[0,16)		16个链
	...
	static inline size_t Roundup(size_t bytes)
	{
		...
		if (bytes <= 128) //向上对齐 8Bytes
		{
			return _Roundup(bytes, 3);
		}
		else if (bytes <= 1024) //向上对齐 16Bytes
		{
			return _Roundup(bytes, 4);
		}
		else if (bytes <= 8192) //向上对齐 128Bytes
		{
